...
class ViewController: UIViewController {
    ...
    var autoCompletePossibilities = ["$AALI", "$aada", "@donny", "@romdoni", "$IHSG", "$SYLA", "@test", "$test"]
...
extension ViewController: UITextViewDelegate {
    ...
    func textView(_ textView: UITextView, shouldChangeTextIn range: NSRange, replacementText text: String) -> Bool {
        ...
        if let result: (tag: String, range: NSRange) = AutoSuggest.sharedInstance.lookingForTag(substring, position: range) {
            AutoSuggest.sharedInstance.foundRange = result.range
            
            for val in autoCompletePossibilities {
...
extension ViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        string = string.replacingCharacters(in: AutoSuggest.sharedInstance.foundRange, with: selectedWord) as NSString
        ...
        if let cursorLocation: UITextPosition = self.textView.position(from: positionOriginal, offset: AutoSuggest.sharedInstance.foundRange.location + selectedWord.characters.count) {
